...
namespace Microsoft.Xna.Framework.Graphics
{
	...
	public class SpriteBatch : GraphicsResource
	{
        ...
        // Stardew references these SpriteBatch globals directly.
        ...
        void _TuckTextureCoordinates(Texture2D texture, ref Vector2 tl, ref Vector2 br)
        {
            if (TextureTuckAmount <= 0f)
                return;

            var tuckX = texture.TexelWidth * TextureTuckAmount;
            var tuckY = texture.TexelHeight * TextureTuckAmount;

            if (Math.Abs(br.X - tl.X) > tuckX * 2f)
            {
                tl.X += tuckX;
                br.X -= tuckX;
            }

            if (Math.Abs(br.Y - tl.Y) > tuckY * 2f)
            {
                tl.Y += tuckY;
                br.Y -= tuckY;
            }
